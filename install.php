...
$languageFromHTTP = 'zh_CN';   // ← 改这里
$localeFromHTTP   = 'zh_CN';   // ← 改这里
...
function install($adminUsername, $adminPassword, $timezone)
{
    ...
    $data = array(
        $adminUsername => array(
            'nickname'  => $adminUsername, // 也可以保留 'Admin'
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($username === '' || !preg_match('/^[A-Za-z0-9_]{3,20}$/', $username)) {
        $errorText = $L->g('Invalid username'); // 你也可以换成中文提示
        error_log('[ERROR] ' . $errorText, 0);
    } elseif (Text::length($_POST['password']) < 6) {
        $errorText = $L->g('password-must-be-at-least-6-characters-long');
        error_log('[ERROR] ' . $errorText, 0);
    } else {
        install($username, $_POST['password'], $_POST['timezone']);
        redirect(HTML_PATH_ROOT);
    }
}
				...
				?>
					...
					<form id="jsformInstaller" method="post" action="" autocomplete="off">
						...
						<div class="form-group">
							<input type="text" dir="auto" value="" class="form-control form-control-lg" id="jsusername" name="username" placeholder="<?php $L->p('Username') ?>" required>
						</div>
