...
class pluginVersion extends Plugin
{
	...
	public function adminSidebar()
	{
		...
		if ($this->getValue('showCurrentVersion')) {
			$html = '<a id="current-version" class="nav-link" href="' . HTML_PATH_ADMIN_ROOT . 'about' . '">' . $L->get('Version') . ' ' . (defined('CLOUDSUO_PRO') ? '<span class="bi-heart" style="color: #ffc107"></span>' : '') . '<span class="badge bg-warning rounded-pill">' . CLOUDSUO_VERSION . '</span></a>';
		}
		if ($this->getValue('newVersionAlert')) {
			$html .= '<a id="new-version" style="display: none;" target="_blank" href="https://www.cloudsuo.com">' . $L->get('New version available') . ' <span class="bi-bell" style="color: red"></span></a>';
		}
		return $html;
	}
	...
	public function adminBodyEnd()
	{
		if ($this->getValue('newVersionAlert')) {
			...
			// The script is on /cl-plugins/version/js/version.js
